...
func ValidateAttribute(attribute decoder.StructAttribute, options ValidationOptions) []string {
	validations := []string{}
	...
	rules := decoder.GetTagValues(attribute.Field, VALIDATION_TAG_KEYWORD)
	for _, validationRule := range rules {
		...
		// Skip this rule
		if helpers.Contains(options.SkipRules, ruleType) {
			continue
		}

		switch ruleType {
		case CURRENCY:
			f, err := helpers.PointerElement(attribute.Value)
			if err != nil {
				return VALUE_ERROR
			}
...
func IsIn(value reflect.Value, acceptedValues []string) bool {
	switch value.Kind() {
	case reflect.String:
		return helpers.Contains(acceptedValues, value.String())
